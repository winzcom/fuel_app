            ...
            <div class="col-md-12">
                <div class="portlet light bordered">
                    ...
                    <div class="portlet-body">
                        <div class="row">
                            <div class="col-md-10">
                                {!! Form::open(['route'=>'reading-search','method'=>'post','class'=>'form-inline margin-bottom-5']) !!}
                        ...
                        <table class="table table-striped table-bordered table-hover" id="sample_1">
                            ...
                            @foreach($reading as $p)
                                ...
                                <tr>
                                    <td>{{ $i }}</td>
                                    <td>{{ date('d-F-y',strtotime($p->start_reading_time)) }}</td>
                                    <td>{{ $p->machine->name }}</td>
                                    <td>{{ $p->machine->fuel->name }}</td>
                                    <td>{{ $p->start_reading }} Litter</td>
                                    <td>{{ $p->current_reading }} Litter</td>
                                    <td>{{ $p->end_reading }} Litter</td>
                                    <td>
                                        @if($p->end_rading != null)
                                            {{ $p->start_reading - $p->end_reading }} / Litter
                                        @else
                                            0 / Litter
                                        @endif
                                    </td>
                                    <td>@php $sell = 0 @endphp
                                        @foreach($invoice as $inv)
                                            @if($inv->pay_date == date('Y-m-d',strtotime($p->start_reading_time)) )
